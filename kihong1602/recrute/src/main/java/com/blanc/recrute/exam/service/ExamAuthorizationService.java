...
public class ExamAuthorizationService implements ExamService {
  ...
  public ValidationDTO validateExamAuthEmail(HttpServletRequest request, AptIdDTO aptIdDTO) {
    ...
    if (requestAuthCookie != null) {
      ...
      return compareAptId(sessionAptId, aptIdDTO);
    } else {
  ...
  private ValidationDTO compareAptId(String sessionAptId, AptIdDTO aptIdDTO) {
    return sessionAptId.equals(aptIdDTO.getAptId()) ? new ValidationDTO(AVAILABLE) : new ValidationDTO(UNAVAILABLE);
  }
}
